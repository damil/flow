package App::Flow::Controller::Explorer;
use utf8;
use strict;
use warnings;
use DBI;
...
sub build_card {
  ...
  : $dbase eq 'psylles'       ? qw(/etc/flow/psyllesexplorer.conf   /etc/flow/psylleseditor.conf  psyllespdf )
  : $dbase eq 'aradides'      ? qw(/etc/flow/aradides.conf          /etc/flow/aradides.conf       aradpdf    )
  : $dbase eq 'coleorrhyncha' ? qw(/etc/flow/peloridexplorer.conf   /etc/flow/pelorideditor.conf  pelopdf    )
  : $dbase eq 'strepsiptera'  ? qw(/etc/flow/strepsexplorer.conf    /etc/flow/strepseditor.conf              )
  : $dbase eq 'aleurodes'     ? qw(/etc/flow/aleurodsexplorer.conf  /etc/flow/aleurodseditor.conf            )
  : $dbase eq 'tingides'      ? qw(/etc/flow/tingides.conf          /etc/flow/tingides.conf                  )
  : $dbase eq 'tessaratomidae'? qw(/etc/flow/tessaratomidae.conf    /etc/flow/tessaratomidae.conf            )
  : $dbase eq 'lucanidae'     ? qw(/etc/flow/lucanidae.conf         /etc/flow/lucanidae.conf                 )
  : $dbase eq 'brentidae'     ? qw(/etc/flow/brentidae.conf         /etc/flow/brentidae.conf                 )
  : $dbase eq 'diptera'       ? qw(/etc/flow/diptera.conf           /etc/flow/diptera.conf                   )
  : $dbase eq 'test'          ? qw(/etc/flow/testexplorer.conf      /etc/flow/testeditor.conf                )
  : $dbase eq 'hefo'          ? qw(/etc/flow/hefo.conf              /etc/flow/hefo.conf                      )
...
sub genera_list {
        ...
        if ( my $dbc = $glob_self->dbh_for($config->{DEFAULT_DB}) ) {
                ...
                if ($ge_numb > 100) {
                  # nombre de taxons par lettre initiale
...
sub taxon_card {
        ...
        if ( my $dbc = $glob_self->dbh_for($config->{DEFAULT_DB}) ) {
                ...
                my $nlabl = $valid_name->[19] ? "&nbsp;$valid_name->[19]" : '';
                ...
                for (my $i=0; $i<$#{$highers}+1; $i++) {
                        ...
                        if (scalar(@{$taxa}) > 1) {
                                ...
                                foreach my $taxon (@{$taxa}) {
                                        my ( $current_id, $current_name, $current_authority ) = @$taxon;

                                        # L. Dami juin 2023: ligne ci-dessous commentée car $found ne peut être que undef
